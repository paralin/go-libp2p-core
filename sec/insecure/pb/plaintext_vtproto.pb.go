...
func (m *Exchange) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.Pubkey != nil {
		if marshalto, ok := interface{}(m.Pubkey).(interface {
			...
			encoded, err := proto.Marshal(m.Pubkey)
			if err != nil {
				return 0, err
			}
			i -= len(encoded)
...
func (m *Exchange) SizeVT() (n int) {
	...
	if m.Pubkey != nil {
		if size, ok := interface{}(m.Pubkey).(interface {
			SizeVT() int
		}); ok {
			l = size.SizeVT()
		} else {
			l = proto.Size(m.Pubkey)
		}
		n += 1 + l + sov(uint64(l))
	}
	if m.unknownFields != nil {
...
func (m *Exchange) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			if unmarshal, ok := interface{}(m.Pubkey).(interface {
				...
				if err := proto.Unmarshal(dAtA[iNdEx:postIndex], m.Pubkey); err != nil {
					return err
				}
			}
			iNdEx = postIndex
